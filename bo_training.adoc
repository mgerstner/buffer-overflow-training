...
* Now we will take a look at security issues related to heap buffer overflows.
* We will only take a cursory look on the heap, as it would justify a
  dedicated training due to its complexity.
...
** Chunks are kept in (sorted) pools or free lists and are linked to each
   other by keeping next/prev pointers in front or back, or both ends of the
   chunks.
* To exploit heap issues, individually tailored exploits are required, because
  the heap is not part of the regular execution flow in a program, as the
  stack is.

Simplified Heap Management Overview
-----------------------------------

image::images/heap_overview.png[align="center"]

Heap Metadata
-------------

Examples for metadata that may be kept in hidden portions of heap memory:

[role="incremental"]
* a pointer to the next chunk, or to some other data structure the chunk is a
  part of.
* state flags e.g. whether the chunk is currently allocated or not, and if any
  special allocation strategy was used.
* the actual payload size of the chunk as visible to the caller of 'malloc()'.

Heap Buffer Overflows
---------------------

What can happen when a heap buffer write overflow occurs?
...
here, when a crafted pointer is dereferenced and written to by the allocator
algorithm).
* If the chunk we can overflow is followed by another heap chunk currently in
  use by the program:
[role="incremental"]
...
** Otherwise (or in any case) we can manipulate some application specific data
   found in the payload of the chunk.
...
buffer overflows, compared to the stack.

Heap Specific Errors (I)
------------------------

Heap memory management involves some additional error types, that don't exist
when dealing with stack memory. These mostly stem from the fact that memory
has to be requested and freed explicitly by the programmer.

[role="incremental"]
=== Memory Leaks

[role="incremental"]
These happen when the programmer forgets to call `free()` when heap memory
is no longer needed by the application.

[role="incremental"]
* Such memory leaks can be used to obtain a range of addresses for keeping
  duplicates of exploit data.
* They also serve a purpose in the stack clash discussed before.
* They are particularly useful for attackers when they can be triggered at
  will in a long-running program (e.g. web service, daemon).
* Ultimately they can lead to Denial-of-Service, when no more memory is
  available for `malloc()`.

Heap Specific Errors (II)
-------------------------
...
[role="incremental"]
When heap memory is `free()'d` but still used by some part of the application
(use after free) then an attacker can attempt to trigger some new heap
allocation that will be placed just into that memory location. This is one of
the most dangerous issues in the heap context.

[role="incremental"]
* Now there live different objects in the application that point to the same
memory. Imagine a user privilege struct being used after free. The attacker
can try to allocate some "harmless" description text on the heap, which will
be placed in the same location. By manipulating the description text, the
attacker now also controls the user privileges used by another part of the
program.
* By skillfully manipulating the data in the heap buffer, it might
be possible to cause unintended program behaviour or even arbitrary code
execution. Each case is individual to the vulnerable application, however.

Heap Specific Errors (III)
--------------------------
...
[role="incremental"]
When a heap pointer is `free()'d` more than once then we will usually observe
a program crash. The heap allocator can often detect this situation ("double
free or corruption").

[role="incremental"]
A double free can also turn into use after free. This happens when a pointer
is `free()'d` once, then another part of the program calls `malloc()` and
receives the pointer previously `free()'d`, which is then wrongly passed a
second time to `free()`.
...
* If the heap chunk was recycled from a previously `free()'d` one then it may
  contain sensitive application data from a different context.
* This data could cause an information leak if read without initializing the
data.
* Applications may base decisions on uninitialized data, resulting in
  undefined and dangerous behaviour.
...
* In combination with information leaks, this can then help finding a suitable
memory address for manipulations to work.
...
space, too.
** The NX bit also applies to the heap, so executing it directly should be
disallowed, although there are other techniques like changing the PLT
(Procedure Lookup Table) of the program, which may allow to bypass such
protections.
* These are measures that could be taken, that depend on the allocator,
  however:
[role="incremental"]
...
A Real-Life Heap Exploitation
-----------------------------

* A year ago we investigated a practical heap vulnerability in PCP (Performance
  Co-Pilot) which yielded interesting results.
* *Hands-on*: see example folder 'pcp_upgrade'.

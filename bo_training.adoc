...
[role="incremental"]
* a pointer to the next chunk, or to some data structure the chunk belongs to.
...
here, when a crafted pointer is dereferenced and written to).
...
The heap has some additional error sources, that don't exist when dealing
with stack memory. These mostly stem from the fact that memory has to be
requested and freed explicitly by the programmer.
...
[role="incremental"]
A double free can also turn into use after free in some circumstances. This
happens when a pointer is `free()'d` once, then another part of the program
calls `malloc()` and receives the pointer previously `free()'d`, which is then
wrongly passed a second time to `free()`.
